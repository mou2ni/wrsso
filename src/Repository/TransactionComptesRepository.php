...
class TransactionComptesRepository extends ServiceEntityRepository
{
    ...
    public function __construct(RegistryInterface $registry)
    {
        ...
        $this->select='jc.id as idJournal, tc.id as tcid, t.id as id,  jc.code as codeJournal, t.dateTransaction as dateTransaction, t.numPiece as numPiece, t.libelle as libelleT,tc.numCompte as numCompte, IDENTITY(tc.compte) as compte, tc.libelle as libelleTC, tc.mDebit, tc.mCredit';
    }
    ...
    public function getBalanceComptes($classe=null, $detail=false, $compteDebut=null, $compteFin=null,$dateDebut=null, $dateFin=null){
        $qb =$this->createQueryBuilder('tc')
            ...
            , SUM(CASE WHEN t.dateTransaction < :dateOuv THEN tc.mDebit ELSE 0 END) as mDebitOuv
            , SUM(CASE WHEN t.dateTransaction < :dateOuv THEN tc.mCredit ELSE 0 END) as mCreditOuv
            , SUM(CASE WHEN t.dateTransaction >= :dateDebut AND t.dateTransaction <= :dateFin THEN tc.mDebit ELSE 0 END) as mDebit
            , SUM(CASE WHEN t.dateTransaction >= :dateDebut AND t.dateTransaction <= :dateFin THEN tc.mCredit ELSE 0 END) as mCredit')
            ->setParameter('dateOuv',$dateDebut)
    ...
    public function getSoldesMvt($dateDebut=null, $dateFin=null)
    {
        $qb = $this->createQueryBuilder('tc')
            ->select('IDENTITY(tc.compte) as compte, tc.numCompte as numCompte 
        ...
        if($dateDebut) {
            $qb->andWhere('t.dateTransaction >= :dateDebut')->setParameter('dateDebut',$dateDebut);
        }
        if($dateFin) {
            $qb->andWhere('t.dateTransaction <= :dateFin')->setParameter('dateFin',$dateFin);
        }
